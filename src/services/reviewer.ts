 ...
 */
function buildReviewSummary(summary: string, commentsCount: number, model: string): string {
  ...
  text += `---\n*Automated review powered by [Codelens](https://github.com) • Model: \`${model}\`*`;
  return text;
}
 ...
 */
function buildFallbackSummary(baseSummary: string, comments: GeminiComment[]): string {
...
export async function runReview(payload: WebhookPayload): Promise<ReviewExecutionResult> {
  ...
  // 5. Post comments back to GitHub as an inline Review
  ...
  const topLevelSummary = buildReviewSummary(aiResponse.summary, validComments.length, modelName);

  console.log(`[AI Reviewer] Submitting review to GitHub: ${owner}/${repo}#${pullNumber} (${reviewComments.length} comments)...`);
  ...
  try {
    ...
    reviewPosted = true;
    console.log(`[AI Reviewer] ✅ Successfully posted review comments to GitHub PR #${pullNumber}!`);
  } catch (postError: any) {
    console.error(`[AI Reviewer] Failed to post inline review comments:`, postError?.message || postError);
    ...
    if (postError?.status === 422 && reviewComments.length > 0) {
      try {
        console.log('[AI Reviewer] Attempting fallback: posting feedback as single review body...');
        const fallbackSummary = buildFallbackSummary(topLevelSummary, validComments);
        await postReviewComments(owner, repo, pullNumber, headSha, [], fallbackSummary);
        reviewPosted = true;
        console.log('[AI Reviewer] ✅ Fallback review body posted successfully!');
      } catch (fallbackError: any) {
        console.error('[AI Reviewer] Fallback review posting failed:', fallbackError?.message);
      }
    }
  }
